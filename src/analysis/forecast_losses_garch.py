#!/usr/bin/env python3
"""
forecast_losses_garch.py - forecast losses vs GARCH under the paper's exact protocol,
with instrument-day block-bootstrap inference, run from event_panel.parquet alone.

Protocol replicated from return_history_benchmarks.py (the script behind the published
...
Losses:
  ...
  winsorized MSE on dx^2 (train 0.995) - variance loss, ladder-consistent
Inference: per-(instrument, date) fold means; instrument-clustered bootstrap (1000).

Also emits two precomputed files into data_export/:
...
# ---- GARCH machinery, verbatim from return_history_benchmarks.py ----
