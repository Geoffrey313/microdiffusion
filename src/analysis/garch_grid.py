#!/usr/bin/env python3
"""
garch_grid.py - full predictive-density grid:
...
def main():
    ...
    bysym.to_csv(DIAG_DIR / "tables" / "sde_garch_grid_bysym.csv", index=False)

    grid = pd.DataFrame({L: [bysym[f"{m}|{L}"].mean() for m in MODELS] for L in INNOV}, index=MODELS)
    grid.to_csv(DIAG_DIR / "tables" / "sde_garch_grid.csv")
    ...
    pd.DataFrame(crows).to_csv(DIAG_DIR / "tables" / "sde_garch_calibration.csv", index=False)
