...
Repeats the diffusion-surface transfer test on the external one-second
cryptocurrency sample. For each instrument the days are walked forward (train on
the W prior day-blocks, test on the next), the one-step diffusion surface
...
This is the reproduction of the two numbers reported in Appendix E:
  ...
  - per-instrument distributions of the relative spread, imbalance, absolute
    one-step return, and top-of-book depth.
...
Inputs : CRYPTO_DIR/{BTC,ETH,ADA}_1sec.csv via data/crypto_panel.py.
...
from common.paths import DIAG_DIR, ensure, load_config
...
def descriptives(clean):
    """Per-asset distributions of spread (bps), imbalance, |return| (bps), depth (notional)."""
    rows = []
    for sym in sorted({s for s, _ in clean}):
        ...
        imb = (bsz - asz) / (bsz + asz)
        ret_bps = 1e4 * np.abs(np.diff(np.log(mid), prepend=np.log(mid)[0]))
        depth = bsz + asz
        for var, x in [("Spread (bps)", spread_bps), ("Imbalance", imb),
                       ("|Return| (bps)", ret_bps), ("Depth (notional)", depth)]:
            r = describe(x)
