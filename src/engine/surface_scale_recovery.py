#!/usr/bin/env python3
"""
surface_scale_recovery.py - test whether the GH likelihood recovers a scale CONSISTENT with a_xx.
...
Run: python3 surface_scale_recovery.py
"""
...
def main():
    ...
    pd.DataFrame({"sqrt_a_xx": rms_scale, "gh_ml_std": gh_scale}).to_csv(
        DIAG_DIR / "tables" / "sde_scale_recovery_summary.csv", index=False)
    pd.DataFrame([{"spearman": sp, "spearman_lo": sp_ci[0], "spearman_hi": sp_ci[1],
        ...
        DIAG_DIR / "tables" / "sde_scale_recovery_stats.csv", index=False)
