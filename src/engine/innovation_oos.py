#!/usr/bin/env python3
"""
innovation_oos.py - fresh-split, OUT-OF-SAMPLE validation of the GH tail law.
...
from common.plot_style import ACCENT_DARK, WARN, MUTED, POS, INK, finish, setup_mpl, despine
...
def main():
    ...
    pd.DataFrame([{k: v for k, v in r.items() if k not in ("ks", "emp", "gh", "norm")}
                 for r in (rAB, rBA)]).to_csv(DIAG_DIR / "tables" / "sde_gh_oos.csv", index=False)
