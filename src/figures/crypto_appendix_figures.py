#!/usr/bin/env python3
"""
crypto_appendix_figures.py - the two Appendix E figures for the external sample.

Mirrors, on the event-time cryptocurrency panel, the two QSE views used to read
the external check:

  fig_crypto_axx        (a) the pooled diffusion surface a_xx(I,S) as a heatmap
  ...
  fig_crypto_heavytail  the pooled held-out standardised increment
...
analysis/crypto_transfer.py exactly, so the figures and the reported numbers are the
same estimator. The feed is external (see common.paths.CRYPTO_DIR); if it is absent
...
# Five distinct instrument colours for the transfer scatter, kept within the
# restrained manuscript palette plus one muted green for the fifth series.
SYM_COLOURS = {"BTC-USD": ACCENT, "ETH-USD": WARN, "ZEC-USD": INK,
               "ONDO-USD": ACCENT_LIGHT, "XLM-USD": "#4f7a4f"}
...
def block_cell_dx(state_df, n_I, m_S):
    ...
    the packing cell = i_bin * m_S + (S_tick - 1) used by crypto_transfer.py."""
...
def figure_surface(states, n_I, m_S, plt):
    ...
    for sym, g in pts.groupby("symbol"):
        b.loglog(g["a_train_n"], g["a_test_n"], "o", ms=3.2, alpha=0.5,
                 color=SYM_COLOURS.get(sym, MUTED), label=sym.replace("-USD", ""))
...
def main():
    if not available():
        print("[crypto-fig] external cryptocurrency feed not found; skipping the "
              "Appendix E figures (see data/README.md).")
        return
    cfg = load_config()
    ...
    print(f"[crypto-fig] fig_crypto_axx: {npts} transferred cells, fold-averaged Spearman {rho:.3f}")
    print(f"[crypto-fig] fig_crypto_heavytail: {nz:,} held-out residuals, Student-t nu={nu:.1f}")
